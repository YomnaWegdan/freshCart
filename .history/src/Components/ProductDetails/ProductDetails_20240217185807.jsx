...
import { getProductDetails, useProducts } from '../../CustomHooks/useProducts';
...
export default function ProductDetails() {
  ...
  let {isLoading , isError, error , data , isFetched} = useProducts('productDetails' , ()=>getProductDetails(id));
  console.log(data)
    ...
    let {isLoading , isError, error , data , isFetched} usePostCart(addToCart)
        ...
        <div className="row justify-content-center align-items-center  vh-100 ">
          ...
          <Slider {...settings}>
              {data.images.map((image)=> 
                <img src={image} alt={data.title} key={data.id} className="w-100"/>)}
          </Slider>
          </div> 
          ...
          <button onClick={()=> postToCart(data.id)} className="btn bg-main text-main-light w-100">Add to cart</button>
