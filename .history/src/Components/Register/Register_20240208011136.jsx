...
export default function Register() {
  ...
  let validationSchema = Yup.object({
    ...
    email:Yup.string().required('email is required').email('invalid password'),
    password:Yup.string().required('password is required').matches(/^[A-Z][\w@]{5,9} $/, 'invalid password ex(Ali@123)'),
    rePassword:Yup.string().required('rePassword is required').oneOF([Yup.ref('password'),'repassword not match woth password']),
    phone:Yup.string().required('phone is required').matches(/^[1]$/, 'invalid phone'),
